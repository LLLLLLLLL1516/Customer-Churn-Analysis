...
"""
To run this:
    in consol: set FLASK_APP=Flask_App.py
    set FLASK_DEBUG=1
    FLASK RUN
"""
...
def predict():
    ...
    new_df['MonthlyCharges'] = pd.to_numeric(new_df['MonthlyCharges'])
    new_df['TotalCharges'] = pd.to_numeric(new_df['TotalCharges'])
    new_df['SeniorCitizen'] = int(pd.to_numeric(new_df['SeniorCitizen']))
    ...
    cat_cols = df_2.columns['gender', 'Partner', 'Dependents', 'PhoneService', 'MultipleLines',
       'InternetService', 'OnlineSecurity', 'OnlineBackup', 'DeviceProtection',
       'TechSupport', 'StreamingTV', 'StreamingMovies', 'Contract',
       'PaperlessBilling', 'PaymentMethod', 'tenure_group']
    num_cols = df_2.columns['SeniorCitizen', 'MonthlyCharges', 'TotalCharges']
    ...
    enc_ohe.fit(df_2[categories])
    
    df_2 = OneHotEncoding(df_2, enc_ohe, categories)
    
    
    from sklearn.preprocessing import StandardScaler
    scaler = StandardScaler()
    scaler.fit(df_2[num_cols])
    df_2[num_cols] = scaler.transform(df_2[num_cols])
